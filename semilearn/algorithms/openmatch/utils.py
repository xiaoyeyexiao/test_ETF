...
# 在multi-binary classifier中，全连接层有2*num_classes个输出结点，每个类对应两个结点
def ova_loss_func(logits_open, label):
    # Eq.(1) in the paper
    # 将 logits_open 的形状重塑为 (2*batch_size, 2, num_classes)
    # 例如 (128, 12) -> (128, 2, 6)，原向量中每一列的第1个值与第7个值对应，第2个值与第8个值对应，以此类推
...
def etf_ova_loss_func(logits_open, label):
    # 使用softmax函数计算每个类所对应的两个结点
    logits_open = F.softmax(logits_open, 1)
    # 形状为 (2*batch_size, num_classes) 的全零张量
    label_s_sp = torch.zeros((logits_open.size(0),
